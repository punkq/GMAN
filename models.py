...
def generator(gan):
    with tf.variable_scope('generator'):
        ...
        with tf.variable_scope('deconv0'):
            f = tf.Variable(tf.truncated_normal([3, 3, int(gan.num_hidden), int(gan.num_latent)], mean=0.0,
            ...
            h0 = tf.nn.bias_add(tf.nn.conv2d_transpose(gan.z, f,
                                                       [int(gan.channel_size), 4, 4, int(gan.num_hidden)],
                                                       strides=[1, 4, 4, 1]), b)
            h0 = batch_norm(h0, gan.num_hidden)
            h0 = tf.nn.relu(h0)

        with tf.variable_scope('deconv1'):
            f = tf.Variable(tf.truncated_normal([5, 5, int(gan.num_hidden / 2), int(gan.num_hidden)], mean=0.0,
                                                stddev=0.02, dtype=tf.float32),
                            name='filter')
            b = tf.Variable(tf.zeros([gan.num_hidden / 2], dtype=tf.float32), name='b')
            h1 = tf.nn.bias_add(tf.nn.conv2d_transpose(h0, f,
                                                       [int(gan.channel_size), 8, 8, int(gan.num_hidden / 2)],
                                                       strides=[1, 2, 2, 1]), b)
            h1 = batch_norm(h1, gan.num_hidden / 2)
            h1 = tf.nn.relu(h1)

        with tf.variable_scope('deconv2'):
            f = tf.Variable(tf.truncated_normal([5, 5, int(gan.num_hidden / 4), int(gan.num_hidden / 2)], mean=0.0,
                                                stddev=0.02, dtype=tf.float32),
                            name='filter')
            b = tf.Variable(tf.zeros([gan.num_hidden / 4], dtype=tf.float32), name='b')
            h2 = tf.nn.bias_add(tf.nn.conv2d_transpose(h1, f,
                                                       [int(gan.channel_size), 16, 16, int(gan.num_hidden / 4)],
                                                       strides=[1, 2, 2, 1]), b)
            h2 = batch_norm(h2, gan.num_hidden / 4)
            h2 = tf.nn.relu(h2)

        with tf.variable_scope('gen_images'):
            f = tf.Variable(tf.truncated_normal([5, 5, int(gan.num_channels), int(gan.num_hidden / 4)], mean=0.0,
                                                stddev=0.02, dtype=tf.float32),
                            name='filter')
            ...
            gen_image = tf.nn.tanh(
                tf.nn.bias_add(tf.nn.conv2d_transpose(h2, f,
                                                      [int(gan.channel_size), int(gan.side), int(gan.side),
                                                       int(gan.num_channels)],
                                                      strides=[1, 2, 2, 1]), b))
    return gen_image
    ...
    with tf.device('/gpu:%d' % gpu_num):
        with tf.variable_scope('discriminator_%d' % num):
            ...
            with tf.variable_scope('conv0'):
                h0 = conv2d(inp, [3, 3, int(gan.num_channels), int(hidden_units / 4)], [int(hidden_units / 4)],
                            stride=2, name='h0')
                h0 = leaky_relu(0.2, h0)
                h0 = tf.nn.dropout(h0, keep_prob)
            with tf.variable_scope('conv1'):
                h1 = conv2d(h0, [3, 3, int(hidden_units / 4), int(hidden_units / 2)], [int(hidden_units / 2)],
                            stride=2, name='h0')
                h1 = leaky_relu(0.2, h1)
                h1 = tf.nn.dropout(h1, keep_prob)
            with tf.variable_scope('conv2'):
                h2 = conv2d(h1, [3, 3, int(hidden_units / 2), int(hidden_units)], [int(hidden_units)],
                            stride=1, name='h0')
